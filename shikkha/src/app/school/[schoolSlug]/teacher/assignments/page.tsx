...
export default async function TeacherAssignmentsPage({ params }: PageProps) {
  const { schoolSlug } = await params;
  const membership = await requireRole(schoolSlug, ["CLASS_TEACHER", "SUBJECT_TEACHER", "MADRASA_USTADH"]);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Link href={`/school/${schoolSlug}/admin/assignments`} className={buttonVariants({ variant: "default", size: "sm" })}>
            নতুন অ্যাসাইনমেন্ট
          </Link>
        }
      />
      ...
      {list.length === 0 ? (
        ...
        <div className="grid gap-3">
          {list.map((a) => (
            <Link key={a.id} href={`/school/${schoolSlug}/admin/assignments/${a.id}`}>
