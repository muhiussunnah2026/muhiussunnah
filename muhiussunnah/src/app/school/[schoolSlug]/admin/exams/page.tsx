...
export default async function ExamsListPage({ params }: PageProps) {
        ...
        <section>
          {list.length === 0 ? (
            ...
            <div className="grid gap-3">
              {list.map((e) => (
                <Link key={e.id} href={`/school/${schoolSlug}/admin/exams/${e.id}`} className="group">
        ...
        <aside>
          <Card>
            <CardContent className="p-5">
              ...
              <AddExamForm schoolSlug={schoolSlug} years={yearList} />
              {yearList.length === 0 ? (
