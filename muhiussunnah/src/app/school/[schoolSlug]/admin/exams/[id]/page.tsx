...
export default async function ExamDetailPage({ params }: PageProps) {
  ...
  return (
    <>
      <PageHeader
        breadcrumbs={
          <Link href={`/school/${schoolSlug}/admin/exams`} className="inline-flex items-center gap-1 text-sm hover:text-foreground">
        ...
        actions={<PublishButton schoolSlug={schoolSlug} examId={id} published={exam.is_published} />}
      />
                    ...
                    <TableBody>
                      {routineRows.map((r) => (
                        <TableRow key={r.id}>
                          ...
                          <TableCell>
                            <Link href={`/school/${schoolSlug}/admin/exams/${id}/marks/${r.id}`} className="font-medium text-primary hover:underline">
                              {r.subjects.name_bn}
                            </Link>
                          </TableCell>
                          ...
                          <TableCell className="text-right">
                            <DeleteRoutineButton schoolSlug={schoolSlug} examId={id} rowId={r.id} />
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
            ...
            <aside>
              <Card>
                <CardContent className="p-5">
                  ...
                  <RoutineAddForm schoolSlug={schoolSlug} examId={id} subjects={subjectList} sections={sectionList} />
                </CardContent>
              </Card>
            </aside>
          </div>
              ...
              {routineRows.length === 0 ? (
                ...
                <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
                  {routineRows.map((r) => (
                    <Link
                      key={r.id}
                      href={`/school/${schoolSlug}/admin/exams/${id}/marks/${r.id}`}
                    ...
                    {cards.map((c) => (
                      <TableRow key={c.id}>
                        ...
                        <TableCell className="text-right">
                          {c.students ? (
                            <Link
                              href={`/school/${schoolSlug}/admin/exams/${id}/marksheet/${c.students.id}`}
