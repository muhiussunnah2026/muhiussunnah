...
export default async function InvoicesListPage({ params, searchParams }: PageProps) {
  ...
  return (
    <>
      ...
      />
      <FeesSubNav schoolSlug={schoolSlug} active="invoices" />
      ...
      <div className="mt-4 grid gap-6 lg:grid-cols-[1fr_360px]">
                  ...
                  <TableBody>
                    {invoices.map((inv) => (
                      <TableRow key={inv.id} className="cursor-pointer">
                        <TableCell>
                          <Link href={`/school/${schoolSlug}/admin/fees/invoices/${inv.id}`} className="font-mono text-xs underline-offset-4 hover:underline">
                            {inv.invoice_no}
                          </Link>
                        ...
                        <TableCell>
                          {inv.students ? (
                            <Link href={`/school/${schoolSlug}/admin/students/${inv.students.id}`} className="font-medium hover:underline">
        ...
        <aside>
          <GenerateInvoicesPanel schoolSlug={schoolSlug} classes={classes ?? []} />
        </aside>
      </div>
    </>
  );
