...
export default async function InvoiceDetailPage({ params }: PageProps) {
  ...
  return (
    <>
      <PageHeader
        breadcrumbs={
          <Link href={`/school/${schoolSlug}/admin/fees/invoices`} className="inline-flex items-center gap-1 text-sm hover:text-foreground">
          ...
          {dueAmount > 0 ? (
            <Card className="border-primary/30">
              <CardContent className="p-5">
                ...
                <RecordPaymentForm schoolSlug={schoolSlug} invoiceId={invoice.id} maxAmount={dueAmount} />
              </CardContent>
            </Card>
          ) : null}
        </aside>
