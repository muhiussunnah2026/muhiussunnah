...
export default async function DuesAgingReportPage({ params }: PageProps) {
            ...
            <Table>
              ...
              <TableBody>
                {buckets[b].map((inv) => (
                  <TableRow key={inv.id}>
                    <TableCell>
                      <Link href={`/school/${schoolSlug}/admin/fees/invoices/${inv.id}`} className="font-mono text-xs text-primary hover:underline">
                        {inv.invoice_no}
                      </Link>
                    </TableCell>
