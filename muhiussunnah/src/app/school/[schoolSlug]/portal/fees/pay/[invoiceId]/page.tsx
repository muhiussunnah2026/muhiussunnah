...
type PageProps = { params: Promise<{ schoolSlug: string; invoiceId: string }> };

export default async function PayInvoicePage({ params }: PageProps) {
  const { schoolSlug, invoiceId } = await params;
  ...
  return (
    <>
      <PageHeader
        breadcrumbs={
          <Link href={`/school/${schoolSlug}/portal/fees`} className="inline-flex items-center gap-1 text-sm hover:text-foreground">
        ...
        <aside className="flex flex-col gap-4">
          <Card className="border-primary/30">
            <CardContent className="p-5">
              ...
              <PayButtons schoolSlug={schoolSlug} invoiceId={invoice.id} gateways={gateways} />
