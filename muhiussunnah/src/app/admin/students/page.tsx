    ...
    <>
      ...
      {students.length === 0 ? (
        <EmptyState
          ...
          primaryAction={
            <Link href={`/admin/students/new`} className={buttonVariants({ className: "bg-gradient-primary text-white" })}>
              নতুন ভর্তি
            </Link>
          }
          secondaryAction={
            <Link href={`/admin/students/bulk-import`} className={buttonVariants({ variant: "outline" })}>
              Excel থেকে import
            </Link>
          }
              ...
              <TableBody>
                {students.map((s) => (
                  <TableRow key={s.id} className="cursor-pointer">
                    <TableCell>
                      <Link href={`/admin/students/${s.id}`} className="flex items-center gap-3 hover:underline-offset-4 hover:underline">
