...
type PageProps = { params: Promise<{ id: string; examSubjectId: string }> };

export default async function MarksEntryPage({ params }: PageProps) {
  const { id: examId, examSubjectId } = await params;
  const membership = await requireActiveRole([...ADMIN_ROLES, "ACCOUNTANT", ...TEACHER_ROLES]);
    ...
    <>
      ...
      {studentList.length === 0 ? (
        ...
        <MarksGrid
          ...
          initial={existingMap}
          locked={locked} schoolSlug={schoolSlug}
        />
      )}
    </>
  );
